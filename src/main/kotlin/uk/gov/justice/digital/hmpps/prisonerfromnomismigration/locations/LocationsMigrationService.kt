...
fun toMigrationRequest(nomisLocationResponse: LocationResponse, parentId: String?) = NomisMigrateLocationRequest(
...
private fun toLocationType(locationType: String): NomisMigrateLocationRequest.LocationType = when (locationType) {
...
private fun toResidentialHousingType(unitType: LocationResponse.UnitType?): NomisMigrateLocationRequest.ResidentialHousingType? = when (unitType) {
...
private fun toReason(reasonCode: LocationResponse.ReasonCode?): NomisMigrateLocationRequest.DeactivationReason = when (reasonCode) {
...
private fun toAttribute(type: String, code: String): NomisMigrateLocationRequest.Attributes? = when (type) {
  "HOU_SANI_FIT" ->
    when (code) {
