...
@RestController
@RequestMapping("/migrate/staff", produces = [MediaType.APPLICATION_JSON_VALUE])
@Tag(name = "Staff Migration Resource")
@PreAuthorize("hasRole('ROLE_PRISONER_FROM_NOMIS__MIGRATION__RW')")
class StaffMigrationResource(
  private val migrationService: StaffMigrationService,
) {
  @PostMapping
  @ResponseStatus(value = HttpStatus.ACCEPTED)
  @Operation(
    summary = "Starts a staff migration. This migration currently does nothing with the filter",
    description = """
      Starts an asynchronous staff migration process. 
    ...
    requestBody = io.swagger.v3.oas.annotations.parameters.RequestBody(
      content = [
        Content(
          ...
          schema = Schema(implementation = StaffMigrationFilter::class),
        ),
      ],
    ),
    responses = [
