...
class CaseNotesEventListener(
  ...
  fun onMessage(message: String): CompletableFuture<Void?> {
    log.debug("Received case notes event message {}", message)
        ...
        "Notification" -> {
          ...
          if (eventFeatureSwitch.isEnabled(eventType, "casenotes")) {
            when (eventType) {
              ...
              "BOOKING-DELETED" -> caseNotesSynchronisationService.bookingDeleted(sqsMessage.Message.fromJson())
              ...
              else -> log.info("Received a case notes message I wasn't expecting {}", eventType)
            }
          } else {
