...
import uk.gov.justice.digital.hmpps.prisonerfromnomismigration.coreperson.CorePersonSynchronisationBeliefsService
import uk.gov.justice.digital.hmpps.prisonerfromnomismigration.service.NotFoundException

@RestController
@Tag(name = "Religion Repair Resource")
@PreAuthorize("hasRole('ROLE_PRISONER_FROM_NOMIS__UPDATE__RW')")
class ReligionsDataRepairResource(
  ...
  suspend fun repairReligion(@PathVariable prisonNumber: String) {
    try {
      synchronisationService.resynchroniseOffenderBelief(prisonNumber)
    } catch (_: NotFound) {
      throw NotFoundException("Prisoner $prisonNumber not found")
    }
    telemetryClient.trackEvent(
      "core-person-religion-resynchronisation-repair",
      mapOf(
