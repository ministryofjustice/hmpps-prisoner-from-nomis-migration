...
class TransferScheduleMigrationResource(
  ...
  @Operation(
    summary = "Repair all transfer movements for a single prisoner",
    description = "Resyncs a single prisoner to DPS.",
    responses = [
      ApiResponse(
        responseCode = "200",
  ...
  suspend fun repairPrisonerTransferMovements(
    @Schema(description = "The prisoner to resync")
    @PathVariable prisonerNumber: String,
  ) {
    telemetryClient.trackEvent(
      "transfer-scheduler-migration-entity-repair-requested",
      mapOf("offenderNo" to prisonerNumber),
      null,
    )

    migrationService.resyncPrisonerTransferMovements(prisonerNumber)
  }
}
