...
class CourtSchedulerConfiguration(
  ...
  ): WebClient = builder.reactiveAuthorisedWebClient(authorizedClientManager, registrationId = "movements-court-api", url = courtSchedulerUrl, dpsTimeout)

  @Bean
  fun courtSchedulerDpsApiResyncWebClient(
    authorizedClientManager: ReactiveOAuth2AuthorizedClientManager,
    builder: WebClient.Builder,
  ): WebClient = builder.reactiveAuthorisedWebClient(authorizedClientManager, registrationId = "movements-court-api", url = courtSchedulerUrl, dpsResyncTimeout)
