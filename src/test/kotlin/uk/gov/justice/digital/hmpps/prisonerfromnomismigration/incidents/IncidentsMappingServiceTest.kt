...
internal class IncidentsMappingServiceTest {
  ...
  inner class FindLatestMigration {
    ...
    internal fun `will supply authentication token`(): Unit = runBlocking {
      incidentsMappingService.findLatestMigration()

      mappingApi.verify(
        getRequestedFor(
          urlPathEqualTo("/mapping/incidents/migrated/latest"),
        )
          .withHeader("Authorization", WireMock.equalTo("Bearer ABCDE")),
      )
    }
    ...
    internal fun `will return null when not found`(): Unit = runBlocking {
      mappingApi.stubFor(
        get(urlPathEqualTo("/mapping/incidents/migrated/latest")).willReturn(
          aResponse()
            .withHeader("Content-Type", "application/json")
            .withStatus(HttpStatus.NOT_FOUND.value())
            .withBody("""{"message":"Not found"}"""),
      ...
      assertThat(incidentsMappingService.findLatestMigration()).isNull()
    }
    ...
    internal fun `will return the mapping when found`(): Unit = runBlocking {
      ...
      val mapping = incidentsMappingService.findLatestMigration()
      assertThat(mapping).isNotNull
    ...
    internal fun `will throw exception for any other error`() {
      mappingApi.stubFor(
        get(urlPathMatching("/mapping/incidents/migrated/latest")).willReturn(
          aResponse()
            .withHeader("Content-Type", "application/json")
            .withStatus(HttpStatus.INTERNAL_SERVER_ERROR.value())
            .withBody("""{"message":"Tea"}"""),
  ...
  inner class GetMigrationDetails {
    ...
    @Test
    internal fun `will supply authentication token`(): Unit = runBlocking {
      incidentsMappingService.getMigrationDetails("2020-01-01T10:00:00")
    ...
    internal fun `will return the mapping when found`(): Unit = runBlocking {
      ...
      val mapping = incidentsMappingService.getMigrationDetails("2020-01-01T10:00:00")
      assertThat(mapping).isNotNull
