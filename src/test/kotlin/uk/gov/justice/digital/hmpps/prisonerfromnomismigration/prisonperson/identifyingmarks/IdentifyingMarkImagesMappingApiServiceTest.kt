...
@Import(
  IdentifyingMarkImagesMappingApiService::class,
  IdentifyingMarkImagesMappingApiMockServer::class,
  PrisonPersonConfiguration::class,
)
class IdentifyingMarkImagesMappingApiServiceTest {
  @Autowired
  private lateinit var apiService: IdentifyingMarkImagesMappingApiService

  @Autowired
  private lateinit var mappingApi: IdentifyingMarkImagesMappingApiMockServer

  @Nested
  inner class GetByNomisId {
    @Test
    internal fun `will pass oath2 token to service`() = runTest {
      mappingApi.stubGetMappingByNomisId(offenderImageId = 23456L)

      apiService.getByNomisId(offenderImageId = 23456L)
    ...
    internal fun `will pass NOMIS ids to service`() = runTest {
      ...
      val mapping = apiService.getByNomisId(offenderImageId = 23456L)

      with(mapping!!) {
        assertThat(nomisOffenderImageId).isEqualTo(23456L)
    ...
    fun `will return null if mapping does not exist`() = runTest {
      ...
      assertThat(apiService.getByNomisId(offenderImageId = 23456L)).isNull()
    }
  ...
  inner class CreateMapping {
    private fun aMapping() = IdentifyingMarkImageMappingDto(
    ...
    internal fun `will pass DPS id to service`() = runTest {
      ...
      mappingApi.verify(
        postRequestedFor(anyUrl())
          .withRequestBody(matchingJsonPath("nomisOffenderImageId", equalTo("23456")))
