...
import uk.gov.justice.digital.hmpps.prisonerfromnomismigration.nomismappings.model.AdvanceMappingDto.MappingType.NOMIS_CREATED
...
@Import(AdvancesMappingService::class, AdvancesMappingApiMockServer::class)
class AdvancesMappingApiServiceTest {
  @Autowired
  private lateinit var apiService: AdvancesMappingService

  @Autowired
  private lateinit var mockServer: AdvancesMappingApiMockServer
  ...
  inner class Advances {
    ...
    inner class GetByNomisIdOrNull {
      ...
      internal fun `will pass oauth2 token to service`() = runTest {
        mockServer.stubGetAdvanceByNomisIdOrNull(nomisAdvanceId = 12345L)
      ...
      internal fun `will pass NOMIS id to service`() = runTest {
        mockServer.stubGetAdvanceByNomisIdOrNull()

        apiService.getByNomisIdOrNull(nomisId = 12345L)

        mockServer.verify(
          getRequestedFor(urlPathEqualTo("/mapping/advances/nomis-id/12345")),
        )
      }
    ...
    inner class GetByNomisId {
      @Test
      internal fun `will pass oauth2 token to service`() = runTest {
        mockServer.stubGetByNomisId(nomisAdvanceId = 12345L)
    ...
    inner class GetMigrationCount {
      ...
      internal fun `will supply authentication token`(): Unit = runBlocking {
        ...
        mappingApi.verify(
          getRequestedFor(
            urlPathMatching("/mapping/advances/migration-id/.*"),
          )
    ...
    inner class CreateMapping {
      ...
      internal fun `will pass oauth2 token to migrate endpoint`() = runTest {
        ...
        mockServer.verify(
          postRequestedFor(urlPathEqualTo("/mapping/advances")).withHeader(
            "Authorization",
