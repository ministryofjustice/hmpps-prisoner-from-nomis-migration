...
class SentencingSynchronisationIntTest : SqsIntegrationTestBase() {
  ...
  inner class CourtSentencingCloneBookingSentenceAdjustmentSynchronisation {
    @Nested
    inner class WhenSingleAdjustmentInSingleSentence {
      @BeforeEach
      fun setUp() {
        ...
        courtSentencingOffenderEventsQueue.sendMessage(
          SQSMessage(
            ...
            Message = OffenderCaseBookingResynchronisationEvent(
              offenderNo = OFFENDER_NUMBER,
              // obviously makes no sense for these to be empty - but we are only testing adjustments so we can leave them empty
              caseIds = emptyList(),
              casesMoved = emptyList(),
              fromBookingId = 54321,
              toBookingId = BOOKING_ID,
              sentenceAdjustments = listOf(
                SentenceIdAndAdjustmentType(
                  sentenceId = SentenceId(
                    offenderBookingId = BOOKING_ID,
