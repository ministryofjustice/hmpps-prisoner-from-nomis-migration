...
class ContactPersonDpsApiMockServer : WireMockServer(WIREMOCK_PORT) {
  companion object {
    ...
    fun getContactResponse() = GetContactResponse(
      id = 12345,
      lastName = "Smith",
      firstName = "John",
      isDeceased = false,
  ...
  fun stubCreatePerson(personResponse: GetContactResponse = getContactResponse()) {
    stubFor(
      post("/contact")
        .willReturn(
          aResponse()
            ...
            .withBody(ContactPersonDpsApiExtension.objectMapper.writeValueAsString(personResponse)),
        ),
    )
  }
