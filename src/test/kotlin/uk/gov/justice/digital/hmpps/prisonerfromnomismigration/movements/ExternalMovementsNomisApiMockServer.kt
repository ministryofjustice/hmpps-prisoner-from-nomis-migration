...
class ExternalMovementsNomisApiMockServer(private val objectMapper: ObjectMapper) {
  ...
  private fun absenceReturn() = TemporaryAbsenceReturn(
    ...
    fromAddressDescription = null,
    fromFullAddress = "Absence return full address",
    fromAddressPostcode = "S1 1AA",
    audit = NomisAudit(
      createDatetime = now,
      createUsername = "USER",
    ),
  ...
  fun stubGetTemporaryAbsenceScheduledMovement(
    ...
    response: ScheduledTemporaryAbsenceResponse = scheduledTemporaryAbsenceResponse(startTime = eventTime, applicationId = applicationId, eventId = eventId, addressOwnerClass = addressOwnerClass),
  ) {
    nomisApi.stubFor(
  ...
  fun scheduledTemporaryAbsenceResponse(startTime: LocalDateTime = now, applicationId: Long = 111, eventId: Long = 1, addressOwnerClass: String = "OFF") = ScheduledTemporaryAbsenceResponse(
    bookingId = 12345,
