...
class TransferSchedulerMigrationIntTest(
  ...
  private fun stubMigrationDependencies(entities: Int = 2, resync: Boolean = false) {
    // TODO mock getting the IDs
    (1..entities)
      .map { index -> "A%04dKT".format(index) }
      .forEach { prisonerNumber ->
        transfersNomisApi.stubGetOffenderTransferMovementsByRootOffender(777L, offenderTransferMovementsResponse(offenderNo = prisonerNumber))
        if (resync) {
  ...
  inner class MigrateEntity {
    ...
    fun `should check for existing mappings`() {
      mappingApi.verify(getRequestedFor(urlPathEqualTo("/mapping/transfer-scheduler/A0001KT/ids")))
    }

    @Test
    fun `should get NOMIS court movement details`() {
      transfersNomisApi.verify(getRequestedFor(urlPathEqualTo("/movements/root-offender-id/777/transfer")))
    }
    ...
    fun `should populate DPS transfer`() {
      TransferScheduleDpsApiMockServer.getRequestBody<ResyncTransfersRequest>(
        putRequestedFor(urlPathEqualTo("/resync/transfers/A0001KT")),
      ).apply {
        with(transfers[0]) {
