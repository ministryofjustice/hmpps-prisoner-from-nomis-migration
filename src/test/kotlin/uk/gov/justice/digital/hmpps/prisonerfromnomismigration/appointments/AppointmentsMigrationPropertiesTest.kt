...
internal class AppointmentsMigrationPropertiesTest {
  ...
  private val migrationQueue = HmppsQueue(APPOINTMENTS_QUEUE_ID, sqsClient, "queue", sqsClient, "dlq")

  private val appointmentsMigrationProperties = AppointmentsMigrationProperties(hmppsQueueService, appointmentsMappingService)
  private lateinit var details: Map<String, Any>

  private fun build() =
    Builder()
      .apply { appointmentsMigrationProperties.contribute(this) }
      .let {
        it.build().details["last APPOINTMENTS migration"] as Map<String, Any>
      }
  ...
  inner class BeforeFirstMigration {
    ...
    internal fun setUp(): Unit = runBlocking {
      mockEmptyQueues()
      whenever(appointmentsMappingService.findLatestMigration()).thenReturn(null)
      details = build()
    }
  ...
  inner class DuringAMigration {
    ...
    internal fun setUp(): Unit = runBlocking {
      ...
      whenever(appointmentsMappingService.findLatestMigration()).thenReturn(LatestMigration(migrationId = "2020-01-01T12:00:00"))
      whenever(appointmentsMappingService.getMigrationDetails("2020-01-01T12:00:00")).thenReturn(
        MigrationDetails(
          count = 12_001,
  ...
  private fun mockQueuesWith(messagesOnQueueCount: Long, messagesInFlightCount: Long, messagesOnDLQCount: Long) {
    whenever(hmppsQueueService.findByQueueId(APPOINTMENTS_QUEUE_ID)).thenReturn(migrationQueue)
    whenever(
